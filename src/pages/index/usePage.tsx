...
export default function usePage() {
    ...
    const unrealBloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 0.3, 2, 0.18);
    // unrealBloomPass;
...
function appendBoxs(scene: THREE.Scene) {
    const geoList = [
        [2, 6, 6],
        [3.5, 1, 4],
        [20, 0, 9],
    ...
    const group = geoList
        .map((geo) => getBox(geo[0], geo[1], geo[2]))
        .reduce((group, box) => {
            group.add(box);
            return group;
        }, new THREE.Group());
...
function getBox(x: number, y: number, z: number) {
    ...
    const geometry = new THREE.BoxGeometry(1, z, 1);
    ...
    mesh.position.setY(z / 2);
    mesh.position.setZ(x);
    mesh.position.setX(y);
...
function getExtrudeGeometry(points: number[][], name: string) {
    ...
    points
        ...
        .map((point) => [point[0] - config.baseWidth * 40, point[1] - config.baseHeight * 40])
        .forEach((point, index) => {
...
function getText(font: Font, properties: { adcode: number; name: string; center: number[] }) {
    const geometry = new TextGeometry("我是大仙尊", {
        font,
    });
    const material = new THREE.MeshBasicMaterial({ color: 0x00ff00 });
    const mesh = new THREE.Mesh(geometry, material);
    mesh.position.set(properties.center[0] - config.baseWidth, properties.center[1] - config.baseHeight, 0);
    return mesh;
}
// 基础圆盘
function getBaseBox() {
